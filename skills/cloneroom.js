...
module.exports = function (controller) {
    ...
    controller.hears('^cloneroom','direct_message,direct_mention', function (bot, message) {
        ...
        bot.startConversation(messeg, function (err, convo) {
            var question = "What name do you want for the room?";
            convo.ask(question, [
                {
                    default: true, 
                        ...
                        async.waterfall([
                            function (callback){
                                spark.rooms.create({"title":roomName}, function(err, response){
                                    if (err)
                                    {
                                        convo.say("I'm sorry, there was an error creating the room, Please contact administrator.");
                                        convo.next();
                                    }
                                    else{
                                        var responseObj = JSON.parse(response);
                                        callback(null,reponseObj.id);
                                    }   
                                });
                            },
